...
def calcular_categorias_renda(renda):
...
def calcular_categorias_idade(idade):
...
def prever_inadimplencia():
    try:
        # Obtenha valores das features da requisição
        features = {
        ...
        # Calcule as categorias 'Categorias_Renda' e 'Categorias_Idade'
        features['Categorias_Renda'] = calcular_categorias_renda(features['person_income'])
        features['Categorias_Idade'] = calcular_categorias_idade(features['person_age'])

        # Crie um DataFrame com as features
        df = pd.DataFrame([features])

        # Faça a predição usando seu modelo
